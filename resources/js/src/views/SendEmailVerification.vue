        ...
        <v-col v-if="emailSentStatus == false" lg="4" class="d-flex align-center auth-bg pa-10 pb-0">
          <v-row>
            <v-col cols="12" sm="8" md="6" lg="12" class="mx-auto">
              <v-card flat>
                <v-card-text>
                  <p class="text-2xl font-weight-semibold text--primary mb-2">{{$t('send_verification_email')}}</p>
                </v-card-text>
                ...
                <v-card-text>
                  <v-form ref="form">
                    ...
                    <v-btn block color="primary" @click="sendLink"> {{$t('send')}} </v-btn>
                  </v-form>
                </v-card-text>
      ...
      const sendLink = async () =>{
        if (form.value.validate()){
          if (await confirmAlert()){
              axios.post("/verify/email/resend", {'email': email.value})
              .then(response => {
