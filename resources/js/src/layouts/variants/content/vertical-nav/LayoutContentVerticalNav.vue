<template>
  <layout-content-vertical-nav :nav-menu-items="navMenuItems" :key="0">
    <slot></slot>
    ...
    <template #footer>
      <div class="d-flex justify-space-between">
        <span><a href="/" target="_blank">{{ name }}</a> &copy; {{ new Date().getFullYear() }} </span>
      </div>
    </template>
...
export default {
  ...
  setup() {
    ...
    watch(store.getters.getUser, (newVal,oldVal) => {
    },{deep:true})
