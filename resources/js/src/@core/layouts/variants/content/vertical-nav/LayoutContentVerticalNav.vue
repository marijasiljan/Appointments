<template>
  ...
  >
    <div class="logInBackHeader d-flex" v-if="logInBack != null" @click="loginBackToOwn()" >
      <span>
        {{ $t('you_are_logged_in_as') }}: {{user.email}} ({{user.role}})
      </span>
      <span class="underline">
        {{ $t('log_in_back_to_your_account') }}: {{logInBack.lastUser.email}}
      </span>
    </div>
    <v-navigation-drawer
