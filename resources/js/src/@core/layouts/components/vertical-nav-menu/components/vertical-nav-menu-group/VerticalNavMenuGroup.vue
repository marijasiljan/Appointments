<template>
  ...
  >
    ...
    <template #activator>
      <v-list-item-title v-if="item.titleAction != null"><router-link style="color: rgba(94, 86, 105, 0.87);text-decoration: none;" :to="item.titleAction">{{ t(item.title) }} </router-link></v-list-item-title>
      <v-list-item-title v-else>{{ t(item.title) }}</v-list-item-title>

      <v-list-item-action v-if="item.badge && store.getters[item.badgeName] > 0" class="flex-shrink-0">
        <v-badge :color="item.badgeColor" inline :content="dataNrs[item.badgeName]"></v-badge>
      </v-list-item-action>
    </template>
...
export default {
  ...
  setup(props) {
    ...
    const dataNrs = ref({
      'chatsNr': computed(() => store.getters.getChatsNr),
      'reviewsNr': computed(() => store.getters.getReviewsNr),
      'appointmentsNr': computed(() => store.getters.getAppointmentsNr),
    })
