...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    generateSlots: function generateSlots() {
      var _this = this;
      this.generatedSlots = [];
      var startTimeDate = new Date("2000-01-01T".concat(this.startTime));
      var endTimeDate = new Date("2000-01-01T".concat(this.endTime));
      var slotDuration = 30 * 60 * 1000; // 30 minutes in milliseconds

      this.selectedDays.forEach(function (selectedDay) {
        // Get the index of the selected day (0 for Sunday, 1 for Monday, and so on)
        var selectedDayIndex = _this.days.indexOf(selectedDay);

        // Calculate the start and end times based on the selected day
        var currentDate = new Date(startTimeDate);
        currentDate.setDate(currentDate.getDate() + (selectedDayIndex + 7 - currentDate.getDay()) % 7);
        var endDate = new Date(endTimeDate);
        while (currentDate <= endDate) {
          _this.generatedSlots.push({
    ...
    saveAvailability: function saveAvailability() {
      ...
      this.selectedDays.forEach(function (selectedDay) {
        var interval = {
          day: selectedDay,
          start_time: _this2.startTime,
          end_time: _this2.endTime
        };
